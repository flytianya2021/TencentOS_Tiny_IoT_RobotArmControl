...
#define APPLICATION_TASK_STK_SIZE       1024*2
k_task_t application_task;
__aligned(4) uint8_t application_task_stk[APPLICATION_TASK_STK_SIZE];
//
#define APPLICATION_TASK_STK_SIZE02       1024*2
k_task_t MPU6050_task;
__aligned(4) uint8_t MPU6050_task_stk[APPLICATION_TASK_STK_SIZE02];

#define APPLICATION_TASK_STK_SIZE03      1024*2
k_task_t ADC_task;
__aligned(4) uint8_t ADC_task_stk[APPLICATION_TASK_STK_SIZE03];
...
int main(void)
{
    ...
    LCD_ShowString(5,0,"Robot Arm Controler",RED,BLACK,24,0);
    //PutChinese_strings(0,30,"ÌÚÑ¶»¶Ó­Äú",0);
    //ADC_main(NULL);
    //MPU6050_main(NULL);
   tos_knl_init();


    tos_task_create(&application_task, "application_task", application_entry, NULL, 1, application_task_stk, APPLICATION_TASK_STK_SIZE, 0);
    tos_task_create(&MPU6050_task, "MPU6050_task", MPU6050_main, NULL, 1, MPU6050_task_stk, APPLICATION_TASK_STK_SIZE02, 0);
   // tos_task_create(&ADC_task, "ADC_task", ADC_main, NULL, 4, ADC_task_stk, APPLICATION_TASK_STK_SIZE03, 0);
